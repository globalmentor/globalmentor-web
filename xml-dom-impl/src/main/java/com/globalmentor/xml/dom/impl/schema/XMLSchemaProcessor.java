...
/**
 * Class to process an XML schema XML document and return an XML schema object. Each schema processor instance keeps a map of other schemas keyed to their
 * target namespaces. A schema can be looked up by namespace with a call to <code>getSchema()</code>. This function will, if necessary, automatically call the
 * static function <code>getDefaultSchema()</code>, which retrieves one of the predefined schemas, and adds that schema to the map of schemas for cached
 * retrieval.
...
public class XMLSchemaProcessor {
	...
	/**
	 * Stores a schema keyed to a particular namespace for later retrieval and use by the schema processor.
	...
	/**
	 * Processes a schema found in a particular element hierarchy, and returns a schema object which represents the schema.
	...
	public static XMLSchema processSchema(final Element element) {
		...
		while(schemaComponent != null) { //while there are schema components left
			...
			switch(schemaComponent.getComponentType()) { //see which type of component this is
				...
				default:
					Clogr.getLogger(XMLSchemaProcessor.class).error("Unrecognized component: {}" + schemaComponent); //TODO fix
					break;
			}
	...
	/**
	 * Processes the child schema components of an XML element, returning a list of objects which represent those schema components.
			...
			if(node.getNodeType() == node.ELEMENT_NODE) { //if this is an element
				...
				if(childNamespaceURI != null && childNamespaceURI.equals(XML_SCHEMA_NAMESPACE_URI_STRING)) {
					...
					if(schemaComponent != null) //if we received a valid schema component
						...
						//TODO fix
						Clogr.getLogger(XMLSchemaProcessor.class).trace("Did not recognize element: {}", childElement.getLocalName()); //TODO fix
				}
			}
		}
	...
	/**
	 * Processes an XML element in a schema that represents a component in a schema, returning an object which represents that schema component.
	 * @param ownerSchema The XML schema which will own the processed components.
	 * @param element The XML element representing the schema component.
	 * @return An object which corresponds to the schema component, or null if the element is not recognized.
	 */
	...
	/**
	 * Processes an XML element in a schema that represents an attribute group. No checks are done to ensure this is indeed the correct type of element.
	 * @param ownerSchema The XML schema which will own the processed components.
	 * @param element The XML element representing the schema component.
	...
	/**
	 * Processes an XML element in a schema that represents a schema element. No checks are done to ensure this is indeed the correct type of element.
	 * @param ownerSchema The XML schema which will own the processed components.
	 * @param element The XML element representing the schema component.
	...
	/**
	 * Processes an XML element in a schema that represents an annotation. No checks are done to ensure this is indeed the correct type of element.
	 * @param ownerSchema The XML schema which will own the processed components.
	 * @param element The XML element representing the schema component.
			...
			if(node.getNodeType() == node.ELEMENT_NODE) { //if this is an element
				...
				if(childNamespaceURI != null && childNamespaceURI.equals(XML_SCHEMA_NAMESPACE_URI_STRING)) {
					...
					if(childLocalName.equals(ELEMENT_APPINFO) || childLocalName.equals(ELEMENT_DOCUMENTATION)) {
						...
						//if this is not <appinfo> or <documentation>
						Clogr.getLogger(XMLSchemaProcessor.class).error("Illegal annotation element: {}" + childLocalName); //TODO fix
				}
			}
		}
	...
	/**
	 * Checks the next schema component in the iterator, and if that component is an annotation, assigns the annotation to the named schema component.
	 * @param namedComponent The named schema component which can have an annotation.
	 * @param schemaComponentIterator The iterator to retrieve processed child components.
	 * @return The next schema component, either the one after the annotation (if the first component was an annotation), the first component (if the first
	 *         component was not an annotation), or <code>null</code> if there are no more componnents.
	 */
