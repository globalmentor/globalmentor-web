...
import com.globalmentor.xml.dom.impl.XMLAttribute;
...
public class XMLMutationEvent extends XMLEvent implements MutationEvent {
	...
	/**
	 * Used to identify a secondary node related to a mutation event. For example, if a mutation event is dispatched to a node indicating that its parent has
	 * changed, the <code>relatedNode</code> is the changed parent. If an event is instead dispatched to a subtree indicating a node was changed within it, the
	 * <code>relatedNode</code> is the changed node. In the case of the DOMAttrModified event it indicates the <code>Attr</code> node which was modified, added,
	 * or removed.
	...
	/**
	 * @return The previous value of the <code>Attr</code> node in DOMAttrModified events, or of the<code>CharacterData</code> node in DOMCharDataModified events,
	...
	/**
	 * @return The new value of the <code>Attr</code> node in DOMAttrModified events, or of the <code>CharacterData</code> node in DOMCharDataModified events, or
	...
	/**
	 * @return The name of the changed <code>Attr</code> node in a DOMAttrModified event, or <code>null</code> if not applicable.
	 */
	...
	/**
	 * @return The type of change which triggered the DOMAttrModified event. The values can be <code>MODIFICATION</code>, <code>ADDITION</code>, or
	 ...
	 * @param cancelableArg Whether or not the event's default action can be prevented. This is ignored, as no mutation events can be cancelable
	 */
	...
	/**
	 * Initializes the value of a <code>MutationEvent</code> created through the <code>DocumentEvent</code> interface. This method may only be called before the
	 * <code>MutationEvent</code> has been dispatched via the <code>dispatchEvent</code> method, though it may be called multiple times during that phase if
	 * necessary. If called multiple times, the final invocation takes precedence.
	 ...
	 * @param prevValueArg The <code>Event</code>'s <code>prevValue</code> attribute. This value may be <code>null</code>.
	 * @param newValueArg The <code>Event</code>'s <code>newValue</code> attribute. This value may be <code>null</code>.
	 * @param attrNameArg The <code>Event</code>'s <code>attrName</code> attribute. This value may be <code>null</code>.
	 * @param attrChangeArg The <code>Event</code>'s <code>attrChange</code> attribute.
	 */
	public void initMutationEvent(String typeArg, boolean canBubbleArg, boolean cancelableArg, Node relatedNodeArg, String prevValueArg, String newValueArg,
	 ...
	 * @return The new event.
	 */
	public static XMLMutationEvent createDOMAttrModifiedEvent(final XMLNode modifiedNode, final XMLAttribute modifiedAttribute, final String attrName,
			final short attrChange, final String prevValue, final String newValue) {
		final XMLMutationEvent xmlMutationEvent = new XMLMutationEvent(); //create a new mutation event
