...
<ul>
	<li>Converts <code>&lt;img&gt;</code> elements to <code>&lt;object&gt;</code>
		  elements.</li>
	<li>Converts <code>&lt;applet&gt;</code> elements to <code>&lt;object&gt;</code>
		  elements.</li>
	<li>Trims beginning whitespace (including non-breaking spaces) in block
			elements that are meant to contain text (e.g. <code>&lt;p&gt;</code>) or
			that contain no block child elements.</li>
	<li>Collapses all whitespace between elements to double linefeeds in the root
		<code>&lt;html&gt;</code> element, the <code>&lt;head&gt;</code> element,
		and the <code>&lt;body&gt;</code> element.</li>
	<li>Converts characters in an element requested the symbol font to Unicode
		equivalents.</li>
	<li>Performs any transformation requested by the <code>text-transform</code>
		CSS property on immediate child nodes.</li>
	<li>Converts text with list-like content to either ordered or unordered
		lists (code>&lt;ol&gt;</code> and code>&lt;ul&gt;</code>).</li>
	<li>Groups text with equal left and right margins inside
		<code>&lt;div class="fullIndent"&gt;</code> elements.</li>
</ul>
<p>Attributes are modified in the following manner:</p>
<ul>
	<li>Changes a "name" attribute to an "id" attribute, if the element has no
		  "id" attribute.</li>
...
<ul>
	...
	<li>Empty elements besides <code>&lt;applet&gt;</code>,
		  <code>&lt;img&gt;</code>, <code>&lt;object&gt;</code>, and
			<code>&lt;param&gt;</code>.</li>
	...
	<li><code>&lt;table&gt;</code> elements that contain only one row and
		  only one cell.</li>
</ul>
<p>The following elements and attributes are "excised" by removing the elements
	but promoting the child elements to be children of the removed element's
	parent:</p>
<ul>
	<li>Text containers (e.g. <code>&lt;p&gt;</code>,
		  <code>&lt;span&gt;</code>, <code>&lt;em&gt;</code>) with only whitespace
			or non-breaking-space text content.</li>
	<li><code>&lt;span&gt;</code> elements which contain all the content of their
			parent element (that is, a span that is the only child of an element).</li>
	<li>Lists (<code>&lt;ol&gt;</code> and <code>&lt;ul&gt;</code>) that contain
...
<ul>
	...
	<li>Removes empty elements besides <code>&lt;applet&gt;</code>,
		  <code>&lt;img&gt;</code>, and <code>&lt;object&gt;</code>.</li>
	...
	<li>Converts an element with the "MsoBlockText" or "MsoBodyTextIndent" class
		attribute value to a <code>&lt;blockquote&gt;</code> element.</li>
	<li>Changes the "MsoTitle" class attribute value to "title". The element name is
		changed to <code>&lt;h1&gt;</code> if it isn't already.</li>
</ul>
<p>If Unicode tidying is enabled, the the following conversions are performed:</p>
<ul>
...
<p>Options:</p>
<ul>
	<li><code>CONVERT_UNDERLINE_ITALICS_OPTION</code>: Converts all underline tags
		(<code>&lt;u&gt;</code>) should be converted to italics tags
		(<code>&lt;i&gt;</code>).</li>
	<li><code>EXTRACT_INTERNAL_STYLESHEETS_OPTION</code>: Extracts internal
		stylesheets and stores them as external ones.
	<li><code>NORMALIZE_STYLE_CLASSES_OPTION</code>:
	  <ul>
		  <li>If the class attribute contains "num" and at least "chap" or "title",
				change the class attribute to "chapterNumber". The element name is
				changed to <code>&lt;h1&gt;</code> if it isn't already.</li>
		</ul>
	</li>
</ul>
...
public class XHTMLTidier extends TextUtilities implements XHTMLConstants, XMLCSSConstants, XMLConstants, SymbolEncodingConstants
{
	...
	/**Whether internal stylesheets should be extracted and stored as external
		stylesheets.*/
	...
	private boolean convertUnderlineItalics=CONVERT_UNDERLINE_ITALICS_OPTION_DEFAULT;
		...
		/**@return Whether underline tags (<code>&lt;u&gt;</code>) should be
		  converted to italics tags (<code>&lt;i&gt;</code>).*/
		public boolean isConvertUnderlineItalics() {return convertUnderlineItalics;}

		/**Sets whether underlining should be converted to italics.
		@param newConvertUnderlineItalics <code>true</code> if underline tags
			(<code>&lt;u&gt;</code>) should be converted to italics tags
			(<code>&lt;i&gt;</code>), else <code>false</code>.
		*/
		public void setConvertUnderlineItalics(final boolean newConvertUnderlineItalics) {convertUnderlineItalics=newConvertUnderlineItalics;}

	/**Whether internal stylesheets should be extracted and stored as external
		stylesheets.*/
	private boolean extractInternalStylesheets=EXTRACT_INTERNAL_STYLESHEETS_OPTION_DEFAULT;

		/**@return Whether internal stylesheets should be extracted and stored as external
			stylesheets.*/
		public boolean isExtractInternalStylesheets() {return extractInternalStylesheets;}

		/**Sets whether internal stylesheets should be extracted and stored as external
			stylesheets.
		@param newExtractInternalStylesheets <code>true</code> if internal
			stylesheets should be extracted and stored as external stylesheets.
		*/
		public void setExtractInternalStylesheets(final boolean newExtractInternalStylesheets) {extractInternalStylesheets=newExtractInternalStylesheets;}
	...
	private boolean normalizeStyleClasses=NORMALIZE_STYLE_CLASSES_OPTION_DEFAULT;
		...
		public boolean isNormalizeStyleClasses() {return normalizeStyleClasses;}

		/**Sets whether style class names should be made consistent.
		@param newNormalizeStyleClasses <code>true</code> if the style classes
			should be normalized to consistent values, else <code>false</code>.
		*/
		public void setNormalizeStyleClasses(final boolean newNormalizeStyleClasses) {normalizeStyleClasses=newNormalizeStyleClasses;}
...
//G***del		options=tidyOptions;  //store the options
	...
	/**Tidies a document assumed to be XHTML. The entire document hierarchy is
		first tidied, using any locally declared styles if necessary. Then the
		hierarchy is tidied again for style, removing any unnecessary local style
		declarations.
	...
	*/
	...
	/**Tidies a document assumed to be XHTML. The entire document hierarchy is
		...
		hierarchy is tidied again for style, removing any unnecessary local style
		declarations.
		A context file can be specified for several options which require a
		destination directory, such as extracting styles.
	@param document The document to tidy.
	@param contextFile The file that serves as a context for other generated files;
		usually is the filename of the original document being tidied.
	*/
...
//G***del XMLUtilities.printTree(document, Debug.getOutput()); //G***del
		...
		if(isExtractInternalStylesheets())  //if we should extract internal stylesheets
		{
			...
			{
				final XMLCSSProcessor cssProcessor=new XMLCSSProcessor();	//create a new CSS processor
				cssProcessor.parseInternalStyleSheets((com.garretwilson.text.xml.XMLDocument)document);  //parse the internal stylesheets G***this needs to be fixed so that it doesn't store stylesheets in the document
				if(((com.garretwilson.text.xml.XMLDocument)document).getStyleSheetList().size()>0)  //if there are internal stylesheets G***this is terrible; fix
				{
					if(((com.garretwilson.text.xml.XMLDocument)document).getStyleSheetList().get(0) instanceof CSSStyleSheet) //if the first stylesheet is a CSS stylesheet
					{
						final CSSStyleSheet cssStyleSheet=(CSSStyleSheet)((com.garretwilson.text.xml.XMLDocument)document).getStyleSheetList().get(0);	//get a reference to this CSS stylesheet
...
//G***del Debug.trace("Looking at node: "+i);
		  final Node childNode=childNodes.item(i);  //get a reference to this node
			if(childNode.getNodeType()==Node.TEXT_NODE) //if this is a text node
			{
				...
				final String collapsedData=StringUtilities.collapseEveryChar(data, CharacterConstants.WHITESPACE_CHARS, "\n\n");
				text.setData(collapsedData);  //replace the text data with the collapsed data
			}
		}
	}

	/**Tidies a node of an XHTML document by first tidying all child nodes, then
		tidying the node based upon its node type. More specifically, these steps
		are:
		<ol>
...
//G***del Debug.trace("Finished tidying child nodes for node: ", node.getNodeName());  //G***del
	...
	/**Tidies the style of an element of an XHTML document by first tidying the
		styles of all child nodes, then tidying the style of the element itself.
...
//G***del 	Debug.trace("new style: "+newStyle.getCssText());  //G***del
	...
	/**Tidies the children of a particular node, treating each one as an
		independent node.
		Called from <code>tidyNode()</code>.
...
//G***del		final int childNodeCount=childNodeList.getLength(); //find out how many nodes there are
	...
	/**Tidies the first-level children of a particular node, recognizing meaning
		within groups of nodes. This method locates lists, for example.
		Called from <code>tidyNode()</code>.
...
//G***del System.out.print("Converting Indented Items");  //G***change to Debug.info()
	...
	/**Searches all first-level child block elements for any content which could
		be classified as list items and appropriately turns those items into lists.
		Called from <code>tidyChildGroups()</code>. This method takes the following
		steps to locate list items:
		<ol>
		  <li>Look at each first-level block element child, ignoring list elements.</li>
			<li>If a child begins with an appropriate marker (such as '1' or 'a') and
				some delimiter (such as ')' or '.'), the item is marked as list item
				potential.</li>
			<li>If there are multiple potential list items, and all their markers
				are appropriately sequenced, the elements and all interspersed nodes
				are removed and placed inside an <code>&lt;ol&gt;</code> or
				<code>&lt;il&gt;</code> as appropriate in the same location as the first
				list item.</li>
		  <li>The newly formed list is tidied.</li>
			<li>This procedure is repeated from the beginning, starting at the first
				child, until no new lists have been created.</li>
		</ol>
	@param element The element the children of which should be tidied.
	@see #tidyChildGroups
	*/
...
//G***del	Debug.trace("Found text node: ", data);
						...
						final StringTokenizer stringTokenizer=new StringTokenizer(data, CharacterConstants.WHITESPACE_CHARS);
	//G***del when works					final StringTokenizer stringTokenizer=new StringTokenizer(data, CharacterConstants.WHITESPACE_CHARS+LIST_ITEM_MARKER_DELIMITER_CHARS);
...
//G***del 	Debug.trace("Here's what we received: ", markerString);  //G***del
						...
						else if(markerString.equals(expectedFirstMarkerString)) //if we got what should be the first item in the list, we seem to be starting a new list
						{
							...
							else  //if we come upon a new list (even though it has the same type), and we completed a list previously
...
//G***del				listItemNodeList.clear(); //clear the items in the list so that we can start collecting new list items for a new list
	...
	/**Searches all first-level child block elements for any content which could
		be classified as indented and appropriately turns those items into
		<code>&lt;div class="fullIndent"&gt;</code> elements.
		Called from <code>tidyChildGroups()</code>. This method takes the following
		steps to locate block quotes:
		<ol>
		  <li>Look at each first-level block element child, ignoring list elements.</li>
			<li>If a child element's left and right margins are greater than zero and
				are equal, the item is marked as included in a potential indented section.</li>
			<li>If an element is found with a greater indention, the preceding elements
				are discarded and the process starts over with the new indented
				element(s).</li>
...
//G***del				  searchStartNode=blockquoteElement.getNextSibling();  //we'll search the child nodes again, this time starting with the node right after the one node we found last time
	...
	/**Searches all first-level child block elements for any content which could
		be classified as headers and appropriately turns those items into
		<code>&lt;hX"&gt;</code> elements.
		Called from <code>tidyChildGroups()</code>.</p>
		<p>The following are always considered to be headers:</p>
		<ul>
		...
		<p>The following numbered headings are located and placed in the following
			heading order:</p>
		<ol>
		...
		<p>Lines with all capital letters are counted as subheadings
			(<code>SUB_HEADING</code>, unless:</p>
		<ul>
			<li>They appear after an element ending with ':'.</li>
			<li>They begin with quotes and appear after an element that appears in
				quotes.</li>
		</ul>
		<p>Single lines that are correctly capitalized are counted as title headings
			(<code>TITLE_HEADING</code>).</p>
	@param element The element the children of which should be converted to child
		headings.
	@see #tidyChildGroups
	*/
	protected void convertChildHeadings(final Element element)
	{
		...
		final int headingLevels[]=new int[MAX_HEADING+1];
		...
		int lastPageBreakType=NO_HEADING; //the type of heading that last made a page break
		...
		int lastAttemptedPageBreakType=NO_HEADING; //the type of heading that last attempted to make a page break
		final Document document=element.getOwnerDocument(); //get the owner document of the element
		final String elementNamespace=element.getNamespaceURI(); //get the element's namespace
//G***del Debug.trace("element: ", element.getNodeName());  //G***del
//G***del Debug.trace("element has nodes: ", element.getChildNodes().getLength());  //G***del
...
//G***del Debug.trace("looking at text: ", text); //G***del
				  if(isPageNumber(text))  //if this is a page number
					{
...
*/
					if(headingType==CONTENTS_HEADING) //if this is a contents heading
					{
						contentsHeadingElement=childElement;  //show that we're processing the table of contents
					}
					else if(headingType==NO_HEADING)  //if this is not a heading
					{
						contentsHeadingElement=null; //turn off processing of contents
					}
					else if(headingType>NO_HEADING)  //if we have a hierarchical heading
				  {
...
//G***del					switch(headingType)  //actually process the heading
				  if(headingType==PAGE_BREAK_HEADING)
					{
						...
						if(lastElement==lastAttemptedPageBreakElement && lastAttemptedPageBreakType==PAGE_BREAK_HEADING)
					...
					else if(headingType!=NO_HEADING) //if this is any other type of heading
					{
...
//G***del when works								if(headingLevel==2 || headingType==CHAPTER_HEADING || headingLevels[CHAPTER_HEADING]<0)
								if(headingLevel==2 || headingType<=MAX_SIGNIFICANT_HEADING || headingLevels[CHAPTER_HEADING]<0)
								{
/*G***debug
...
*/
									if(lastElement!=lastAttemptedPageBreakElement
										  || headingType<lastAttemptedPageBreakType
											|| lastAttemptedPageBreakType==PAGE_BREAK_HEADING
											|| isBookTitleHeading)
...
//G***del System.out.println("current spacing: "+(elementIndex-lastPageBreakElementIndex)); //G***del
										...
										//  as are book titles)
										if(headingType<=MAX_SIGNIFICANT_HEADING || elementIndex>lastAttemptedPageBreakElementIndex+2 || lastAttemptedPageBreakElementIndex<=0 || isLastPageBreakHeadingFixed || isBookTitleHeading)
										{
											...
											if(headingLevel<4 && elementIndex>0)
											{
												...
												lastPageBreakElementIndex=elementIndex; //show at what index we made a page break
												lastPageBreakType=headingType;  //show what type of heading we used
												if(lastPageBreakElement==lastElement && lastPageBreakType==PAGE_BREAK_HEADING) //if we're following a hard page break <br/>
													element.removeChild(lastPageBreakElement);  //remove the last page break
											}
										}
									}
...
*/
	...
	@return The first of the resulting elements. This may be the original element
		specified or the first of its replacements.
	*/
...
//G***del Debug.trace("Text data: ", textNode.getData()); //G***del
	...
	@param element The element which may be renamed. The element must have a
		valid parent and a valid owner document.
	@return The original element if the element was not renamed, or an elements
		at the same position as the original element containing the same children
	...
	*/
	...
	/**Checks to see if this element contains one or more first-level break
		(<code>&lt;br&gt;</code>) elements. If so, the current element is cloned
		and its children are divided among the various clones, one for each break
		section. The actual break elements are removed.
	@param element The element to tidy. The element must have a valid parent and
		a valid owner document.
	@return The original element if no breaks were found, or an elements at the
		same position as the original element containing the first child
		representing the content between the breaks.
	*/
...
*/
	...
	/**Tidies the attributes of the element. The element name or position is not
		changed.
...
//G***del when works			element.removeAttributeNS(null, ATTRIBUTE_STYLE); //remove the style attribute
	...
	/**Tidies class-related attributes of an element. If the element needs to be
		replaced, it is replaced, remaining at the same relative position as the
		original element. The new element is returned, either the original or its
		replacement.
...
//G***del XMLUtilities.printTree(parentElement, Debug.getOutput()); //G***del
						...
						else  //if the split text was at the beginning of the node to begin with
...
*/
	...
	/**Creates an <code>&lt;object&gt;</code> element from an
...
//G***del Debug.trace("Looking at child node: "+childNode.getNodeName());
	...
	/**Prepares a node for removal by promoting "id", and/or "name"
		attributes (if this is an element) to either the next sibling, the previous
		sibling, or if there are no	siblings, the parent. "xml:lang", "class" and/or
		"style" elements are only promoted to the parent element, and only if that
		parent has no other child elements.
		If the element to which the attribute values will be promoted already has
		values for attributes with those names, the values for the element being
...
//G***del; this is not useful, and can cause harm (by moving up the symbol font property, for example)					copyAttributeValue(stylePromotionElement, element, null, ATTRIBUTE_STYLE);
	...
	/**Copies the value of an attribute to another element, if the attribute
		exists in the source element but does not exist in the destination element.
		The attribute is not removed from the source element.
...
//G***del				element.removeAttributeNS(null, ATTRIBUTE_NAME);  //remove the name attribute
	...
	/**Prunes any child nodes that are determined to be untidy, promoting their
		children to the same level as the removed node.
	@param element The element the children of which should be examined for
		pruning.
...
*/
	...
	@return <code>true</code> if the node being examined should be removed, along
		with its entire descendant hierarchy.
	*/
...
*/
	...
	@param node The node to examine.
	@return <code>true</code> if the node being examined should be pruned and its
		children promoted to be children of the node's parent.
	*/
...
//G***del		final int childCount=childNodeList.getLength(); //find out how many child nodes there are
	...
	/**Determines if the given node is a block element such as
		<code>&lt;p&gt;</code>, <code>&lt;blockquote&gt;</code>, and
		<code>&lt;div&gt;</code>.
	@param node The node to examine.
	...
	*/
	protected static boolean isBlockElement(final Element element)
	{
		...
		return elementName.equals(ELEMENT_P)
			  ...
			  || elementName.equals(ELEMENT_DIV)
			  || elementName.equals(ELEMENT_H1)
			  || elementName.equals(ELEMENT_H2)
			  || elementName.equals(ELEMENT_H3)
			  || elementName.equals(ELEMENT_H4)
			  || elementName.equals(ELEMENT_H5)
			  || elementName.equals(ELEMENT_H6)
			  || elementName.equals(ELEMENT_LI);
	}

	/**Determines if the given node is a list element such as
	...
	*/
	...
	/**Determines if the given node is a text container, an element that is meant
		to contain text, such as <code>&lt;p&gt;</code>, <code>&lt;span&gt;</code>,
		<code>&lt;em&gt;</code>. Used to determine if a whitespace-only element
		should be removed.
	@param node The node to examine.
...
//G***fix					nodeName.equals(ELEMENT_DIV)  //G***add others, like <em>, <code>, etc.
					...
					|| nodeName.equals(ELEMENT_EM)
					|| nodeName.equals(ELEMENT_I)
					|| nodeName.equals(ELEMENT_STRONG)
					|| nodeName.equals(ELEMENT_SUB)
					|| nodeName.equals(ELEMENT_SUP)
				)
			)
		{
