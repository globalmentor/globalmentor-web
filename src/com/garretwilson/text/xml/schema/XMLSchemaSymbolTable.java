...
the same type category). The table provides for symbol spaces grouped by
namespace. That is, each namespace will have multiple symbol spaces (one for
each type), and within each will appear a separate mapping of names to schema
components.
<p>A schema's symbol table will contain components from all symbol spaces.
A complex type, on the other hand, will only contain elements and attributes.</p>
@see XMLSchema
...
public class XMLSchemaSymbolTable
{
	...
	/**A map of namespaces keyed to namespace URIs. Each value in the map is
		a symbol space, represented by another map keyed to symbol space names. Each
		symbol space is in turn a map keyed to object name.
	*/
	private final Map namespaceSymbolSpaceMapMap=new HashMap();

		/**Returns a symbol space map for a particular namespace. If one does not
		  exist, it will be created and added to the map.
		@param namespace The URI of the namespace for which a map of symbol spaces
			will be returned.
		...
		*/
		...
		/**Returns a symbols space (a map of schema components keyed by name) for a
		  particular symbol category inside a particular namespace. If a symbol space
		...
		@return A symbols space for a particular symbol category inside a particular
			namespace.
		...
		*/
		...
		/**Returns a read-only collection of all components in a symbol space. The
		  returned collection, while read-only, is backed by the symbol space and
			will reflect any modifications to the symbol space.
		  If a symbol space for this symbol category does not exist, it will be
			created and added to the map.
		@param namespace The URI of the namespace for which symbols will be returned.
		@param symbolSpaceName A name representing the category of symbols (such as
			attributes or types) the returned collection will represent.
...
*/
		...
		@param symbolSpaceName A name representing the category of symbols (such as
			attributes or types) the component is in.
		@param symbolName The name of the component to return.
		@return A schema component from the table, or <code>null</code> if no such
			schema component exists.
		*/
		...
		/**Puts a named schema component into the table. The component will reside
		  inside a particular symbol space which in turn resides inside a particular
			namespace.
		@param namespace The URI of the namespace of the component.
