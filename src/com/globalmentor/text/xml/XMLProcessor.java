	...
	*/
	public static CharacterEncoding getXMLEncoding(final InputStream inputStream, final StringBuffer encodingAttributeValue, final StringBuffer autodetectPrereadCharacters) throws IOException
	{
		...
		if(inputStream.read(byteOrderMarkArray)==byteOrderMarkArray.length)	//read the byte order mark; if we didn't reach the end of the data
		{
			...
			if(detectedCharacterEncoding!=null)	//if we detected an encoding, use that one, because it will contain the correct byte order mark (we still have to convert the other characters, if any, to preread characters)
			{
			...
			while(true)	//we now know the encoding family and have a string with the bytes read so far; now, try to find any specified character encoding
			{
				...
				{
						//TODO mabye make this more efficient by createing a CharSequenceUtilities.indexOf() method that takes a character argument
...
/*TODO see how this should really be interpreted; probably don't do this in the parser itself
...
//TODO even worse, we're checking the characters sent to us---won't that always be the XML declarations? and why 
...
//TODO del	@exception ParseEOFException Thrown when the end of the input stream is reached unexpectedly.
	...
	@return The entity referenced by this entity reference or a <code>String</code>
		...
		for well-formedness.
	*/
	protected Object parseEntityReference(final XMLReader reader, final XMLDocument ownerDocument) throws IOException, XMLSyntaxException, XMLWellFormednessException, ParseUnexpectedDataException, XMLUndefinedEntityReferenceException
	{
		...
		try
		{
			...
			if(documentType.getEntityXMLNamedNodeMap().containsKey(entityReferenceName))	//if our map of entity references has this entity name
			{
				...
				if(!entity.isParsedEntity())	//if this is an unparsed entity
					throw new XMLWellFormednessException(XMLWellFormednessException.PARSED_ENTITY, new Object[]{entity.getNodeName()}, reader.getLineIndex(), reader.getCharIndex(), reader.getName());	//show that we don't accept unparsed entity references here TODO we need to give a better position in the error, here
		...
		{
			throw new XMLWellFormednessException(parseEOFException, ResourceBundle.getBundle(XMLResources.RESOURCE_BUNDLE_BASE_NAME).getString(XMLResources.RESOURCE_PREFIX+ENTITY_REFERENCE_RESOURCE_ID), entityReferenceName!=null ? entityReferenceName : ResourceBundle.getBundle(XMLResources.RESOURCE_BUNDLE_BASE_NAME).getString(XMLResources.RESOURCE_PREFIX+UNKNOWN_RESOURCE_ID));	//show that we couldn't find the end of the entity reference
		}
	}
...
//TODO del	@exception ParseEOFException Thrown when the end of the input stream is reached unexpectedly.
	...
	*/
	protected XMLEntity parseParameterEntityReference(final XMLReader reader, final XMLDocument ownerDocument, final XMLNamedNodeMap parameterEntityMap) throws IOException, XMLSyntaxException, XMLWellFormednessException, ParseUnexpectedDataException, XMLUndefinedEntityReferenceException
	{
		...
		{
			throw new XMLWellFormednessException(parseEOFException, ResourceBundle.getBundle(XMLResources.RESOURCE_BUNDLE_BASE_NAME).getString(XMLResources.RESOURCE_PREFIX+PARAMETER_ENTITY_REFERENCE_RESOURCE_ID), entityReferenceName!=null ? entityReferenceName : ResourceBundle.getBundle(XMLResources.RESOURCE_BUNDLE_BASE_NAME).getString(XMLResources.RESOURCE_PREFIX+UNKNOWN_RESOURCE_ID));	//show that we couldn't find the end of the parameter entity reference
		}
	}
...
//TODO del	@exception ParseEOFException Thrown when the end of the input stream is reached unexpectedly.
	...
	@see XMLAttribute
	*/
	protected XMLAttribute parseAttribute(final XMLReader reader, final XMLDocument ownerDocument) throws IOException, XMLSyntaxException, XMLWellFormednessException, XMLUndefinedEntityReferenceException, ParseUnexpectedDataException
	{
...
*/
		  ...
		  String attributeValueDelimiters; //we'll store the ending delimiter for the attribute value in this variable
			...
			if(!parseAttributeValue(reader, ownerDocument, attribute, attributeValueDelimiters))	//parse the attribute; if we couldn't find the end of the attribute value
				throw new XMLWellFormednessException(XMLWellFormednessException.EOF, new Object[]{ResourceBundle.getBundle(XMLResources.RESOURCE_BUNDLE_BASE_NAME).getString(XMLResources.RESOURCE_PREFIX+ATTRIBUTE_RESOURCE_ID), attribute.getNodeName()}, reader.getLineIndex(), reader.getCharIndex(), reader.getName());	//show that we couldn't find the end of the attribute
...
*/
		...
		{
			throw new XMLWellFormednessException(parseEOFException, ResourceBundle.getBundle(XMLResources.RESOURCE_BUNDLE_BASE_NAME).getString(XMLResources.RESOURCE_PREFIX+ATTRIBUTE_RESOURCE_ID), attributeName!=null ? attributeName : ResourceBundle.getBundle(XMLResources.RESOURCE_BUNDLE_BASE_NAME).getString(XMLResources.RESOURCE_PREFIX+UNKNOWN_RESOURCE_ID));	//show that we couldn't find the end of the attribute
		}
	}
						...
						else	//if this is not a character reference, it must be an entity reference
						{
							...
							if(entityResult instanceof XMLEntity)	//if we found a matching entity
							{
								...
								if(nestedEntityReferenceMap.containsKey(entity))	//if, somewhere along our nested way, we've already processed this entity (i.e. this is a circular entity reference)
									throw new XMLWellFormednessException(XMLWellFormednessException.NO_RECURSION, new Object[]{entity.getNodeName()}, reader.getLineIndex(), reader.getCharIndex(), reader.getName());	//show that an entity can't reference itself, directly or indirectly TODO we need to give a better position in the error, here
								...
								final XMLReader entityReader=new XMLReader(entity.getText(), "General entity \""+entity.getNodeName()+"\" from \""+entity.getSourceName()+"\"");	//create a string reader from the text of the entity, giving our entity name for the name of the reader TODO i18n
...
entityReader.tidy=isTidy();  //TODO fix
		...
		{
			throw new XMLWellFormednessException(parseEOFException, ResourceBundle.getBundle(XMLResources.RESOURCE_BUNDLE_BASE_NAME).getString(XMLResources.RESOURCE_PREFIX+ATTRIBUTE_RESOURCE_ID), attribute.getNodeName());	//show that we couldn't find the end of the attribute
		}
	}
	...
	*/
	protected XMLExternalID parseExternalID(final XMLReader reader) throws IOException, XMLWellFormednessException, ParseUnexpectedDataException
	{
		...
		{
			throw new XMLWellFormednessException(parseEOFException, ResourceBundle.getBundle(XMLResources.RESOURCE_BUNDLE_BASE_NAME).getString(XMLResources.RESOURCE_PREFIX+EXTERNAL_ID_RESOURCE_ID), ResourceBundle.getBundle(XMLResources.RESOURCE_BUNDLE_BASE_NAME).getString(XMLResources.RESOURCE_PREFIX+UNKNOWN_RESOURCE_ID));	//show that we couldn't find the end of the external ID
		}
	}
		...
		try
		{
		  ...
		  if(isTidy())  //if we're tidying the document
			...
			if(reader.peekChar()=='[')	//if we're ready to read the internal document type declaration subset
			{
				...
				{
					throw new XMLWellFormednessException(XMLWellFormednessException.EOF, new Object[]{ResourceBundle.getBundle(XMLResources.RESOURCE_BUNDLE_BASE_NAME).getString(XMLResources.RESOURCE_PREFIX+DOCUMENT_TYPE_RESOURCE_ID), documentType.getName()}, reader.getLineIndex(), reader.getCharIndex(), reader.getName());	//show that we couldn't find the end of the document type
				}
...
*/
		...
		{
			throw new XMLWellFormednessException(parseEOFException, ResourceBundle.getBundle(XMLResources.RESOURCE_BUNDLE_BASE_NAME).getString(XMLResources.RESOURCE_PREFIX+DOCUMENT_TYPE_RESOURCE_ID), documentType.getName()!=null ? documentType.getName() : ResourceBundle.getBundle(XMLResources.RESOURCE_BUNDLE_BASE_NAME).getString(XMLResources.RESOURCE_PREFIX+UNKNOWN_RESOURCE_ID));	//show that we couldn't find the end of the document type
		}
	}
	...
	/**A map of DTD subsets being cached, keyed by public ID and/or private ID.
		...
		set to <code>null</code>. TODO eventually allow a special DTD subset object to be stored
	*/
...
//TODO fix	@param documentType The document type being processed.
				...
				case PARAMETER_ENTITY_REF:	//if this is a parameter entity reference
					{
						...
						try
						{
							if(parseDocumentTypeContent(entityReader, ownerDocument, /*TODO fix documentType, */generalEntityMap, parameterEntityMap, elementDeclarationList, attributeListDeclarationList))	//parse the entity text just as if it were included in the file (except for markup across entity boundaries); if the ending markup for the DTD for the element was found TODO fix; how can they find the ']' in a parameter entity?
				...
				case ENTITY_DECL:	//if this is an entity declaration
					{
						...
						{		//TODO do parameter entities have the same action of ignoring multiple definitions?
...
*/
	...
	*/
	...
	//TODO should we pass the text node instead?
			...
			{
				throw new XMLWellFormednessException(XMLWellFormednessException.EOF, new Object[]{ResourceBundle.getBundle(XMLResources.RESOURCE_BUNDLE_BASE_NAME).getString(XMLResources.RESOURCE_PREFIX+ENTITY_DECLARATION_RESOURCE_ID), entity.getNodeName()}, reader.getLineIndex(), reader.getCharIndex(), reader.getName());	//show that we couldn't find the end of the entity declaration
			}
	...
	*/
		...
		{
			throw new XMLWellFormednessException(parseEOFException, ResourceBundle.getBundle(XMLResources.RESOURCE_BUNDLE_BASE_NAME).getString(XMLResources.RESOURCE_PREFIX+ENTITY_DECLARATION_RESOURCE_ID), entity.getNodeName()!=null ? entity.getNodeName() : ResourceBundle.getBundle(XMLResources.RESOURCE_BUNDLE_BASE_NAME).getString(XMLResources.RESOURCE_PREFIX+UNKNOWN_RESOURCE_ID));	//show that we couldn't find the end of the entity declaration
		}
	}
...
//TODO del Debug.trace("Found attribute default.");
	...
	*/
	protected XMLNode parseMarkup(final XMLReader reader, final XMLDocument ownerDocument) throws IOException, XMLSyntaxException, XMLWellFormednessException, XMLValidityException, XMLUndefinedEntityReferenceException, ParseUnexpectedDataException
	{
		...
		{
			throw new XMLWellFormednessException(parseEOFException, ResourceBundle.getBundle(XMLResources.RESOURCE_BUNDLE_BASE_NAME).getString(XMLResources.RESOURCE_PREFIX+MARKUP_RESOURCE_ID), ResourceBundle.getBundle(XMLResources.RESOURCE_BUNDLE_BASE_NAME).getString(XMLResources.RESOURCE_PREFIX+UNKNOWN_RESOURCE_ID));	//show that we couldn't find markup
		}
	}
	...
	*/
	protected XMLTag parseTag(final XMLReader reader, final XMLDocument ownerDocument) throws IOException, XMLSyntaxException, XMLWellFormednessException, ParseUnexpectedDataException, XMLUndefinedEntityReferenceException
	{
		...
		catch(final ParseEOFException parseEOFException)	//if we run out of data
		{
			...
			throw new XMLWellFormednessException(parseEOFException, ResourceBundle.getBundle(XMLResources.RESOURCE_BUNDLE_BASE_NAME).getString(XMLResources.RESOURCE_PREFIX+TAG_RESOURCE_ID), tag.getNodeName()!=null ? tag.getNodeName() : ResourceBundle.getBundle(XMLResources.RESOURCE_BUNDLE_BASE_NAME).getString(XMLResources.RESOURCE_PREFIX+UNKNOWN_RESOURCE_ID));	
		}
	}
	...
	*/
					...
					default:	//any other character means we have more attribute/value pairs
						...
						if(attribute.getNodeName().equals(VERSIONINFO_NAME))	//if this is the version info ("version")
						{
							if(foundVersionInfo)	//if we've already found the version information
								throw new XMLSyntaxException("\""+attribute.getNodeName()+"\" already defined.", attribute.getLineIndex(), attribute.getCharIndex(), reader.getName());	//show that this attribute has already been defined TODO this shouln't be a syntax exception
						...
						else if(attribute.getNodeName().equals(ENCODINGDECL_NAME))	//if this is the encoding declaration ("encoding")
						{
							if(!foundVersionInfo)	//if we haven't yet found the version information
								throw new XMLSyntaxException("Missing \""+VERSIONINFO_NAME+"\" declaration.", attribute.getLineIndex(), attribute.getCharIndex(), reader.getName());	//show that the version info is missing TODO this shouldn't be a syntax exception
							...
							if(foundSDDecl)	//if we've already found the standalone declaration
								throw new XMLSyntaxException("\""+attribute.getNodeName()+"\" out of order.", attribute.getLineIndex(), attribute.getCharIndex(), reader.getName());	//show that this attribute is out of order TODO this shouln't be a syntax exception
							//TODO check the format of the version number here
						...
						{
							if(!foundVersionInfo)	//if we haven't yet found the version information
								throw new XMLSyntaxException("Missing \""+VERSIONINFO_NAME+"\" declaration.", attribute.getLineIndex(), attribute.getCharIndex(), reader.getName());	//show that this attribute has already been defined TODO this shouln't be a syntax exception
							if(foundSDDecl)	//if we've already found the standalone declaration
								throw new XMLSyntaxException("\""+attribute.getNodeName()+"\" already defined.", attribute.getLineIndex(), attribute.getCharIndex(), reader.getName());	//show that this attribute has already been defined TODO this shouln't be a syntax exception
	...
	//TODO fix						xmlDecl.setEncoding(attribute.getNodeValue());	//set the encoding
							...
							throw new XMLSyntaxException("Invalid XML declaration attribute: \""+VERSIONINFO_NAME+"\".", attribute.getLineIndex(), attribute.getCharIndex(), reader.getName());	//show that we don't recognize this attribute TODO this shouldn't be a syntax exception
						break;
				}
			}
		}
		...
		{
			throw new XMLWellFormednessException(parseEOFException, ResourceBundle.getBundle(XMLResources.RESOURCE_BUNDLE_BASE_NAME).getString(XMLResources.RESOURCE_PREFIX+XML_DECLARATION_RESOURCE_ID), xmlDecl.getNodeName()!=null ? xmlDecl.getNodeName() : ResourceBundle.getBundle(XMLResources.RESOURCE_BUNDLE_BASE_NAME).getString(XMLResources.RESOURCE_PREFIX+UNKNOWN_RESOURCE_ID));	//show that we couldn't find the end of the XML declaration
		}
	}
	...
	*/
	protected XMLProcessingInstruction parseProcessingInstruction(final XMLReader reader, final XMLDocument ownerDocument) throws IOException, XMLSyntaxException, XMLWellFormednessException, ParseUnexpectedDataException
	{
		...
		{
			throw new XMLWellFormednessException(parseEOFException, ResourceBundle.getBundle(XMLResources.RESOURCE_BUNDLE_BASE_NAME).getString(XMLResources.RESOURCE_PREFIX+PROCESSING_INSTRUCTION_RESOURCE_ID), processingInstruction.getTarget()!=null ? processingInstruction.getNodeName() : ResourceBundle.getBundle(XMLResources.RESOURCE_BUNDLE_BASE_NAME).getString(XMLResources.RESOURCE_PREFIX+UNKNOWN_RESOURCE_ID));	//show that we couldn't find the end of the XML processing instruction
		}
	}
	...
	*/
	protected XMLCDATASection parseCDATASection(final XMLReader reader, final XMLDocument ownerDocument) throws IOException, XMLSyntaxException, XMLWellFormednessException, ParseUnexpectedDataException
	{
		...
		{
			throw new XMLWellFormednessException(parseEOFException, ResourceBundle.getBundle(XMLResources.RESOURCE_BUNDLE_BASE_NAME).getString(XMLResources.RESOURCE_PREFIX+CDATA_RESOURCE_ID), ResourceBundle.getBundle(XMLResources.RESOURCE_BUNDLE_BASE_NAME).getString(XMLResources.RESOURCE_PREFIX+UNKNOWN_RESOURCE_ID));	//show that we couldn't find the end of the CDATA
		}
	}
	...
	*/
	protected XMLComment parseComment(final XMLReader reader, final XMLDocument ownerDocument) throws IOException, XMLSyntaxException, XMLWellFormednessException, ParseUnexpectedDataException
	{
		...
		{
			throw new XMLWellFormednessException(parseEOFException, ResourceBundle.getBundle(XMLResources.RESOURCE_BUNDLE_BASE_NAME).getString(XMLResources.RESOURCE_PREFIX+COMMENT_RESOURCE_ID), ResourceBundle.getBundle(XMLResources.RESOURCE_BUNDLE_BASE_NAME).getString(XMLResources.RESOURCE_PREFIX+UNKNOWN_RESOURCE_ID));	//show that we couldn't find the end of the comment
		}
	}
	...
	*/
	protected XMLElement parseElement(final XMLReader reader, final XMLDocument ownerDocument, final XMLTag tag) throws IOException, XMLSyntaxException, XMLWellFormednessException, XMLValidityException, XMLUndefinedEntityReferenceException, ParseUnexpectedDataException
	{
		...
		try
		{
			...
			{
				throw new XMLWellFormednessException(XMLWellFormednessException.EOF, new Object[]{ResourceBundle.getBundle(XMLResources.RESOURCE_BUNDLE_BASE_NAME).getString(XMLResources.RESOURCE_PREFIX+ELEMENT_RESOURCE_ID), element.getNodeName()}, reader.getLineIndex(), reader.getCharIndex(), reader.getName());	//show that we couldn't find the end of the element
			}
			return element;	//return the element we constructed
		}
		catch(final ParseEOFException parseEOFException)	//if we run out of data
		{
			throw new XMLWellFormednessException(parseEOFException, ResourceBundle.getBundle(XMLResources.RESOURCE_BUNDLE_BASE_NAME).getString(XMLResources.RESOURCE_PREFIX+ELEMENT_RESOURCE_ID), element.getNodeName());	//show that we couldn't find the end of the element
		}
	...
	*/
	protected boolean isTidyContentModel(final XMLElement parentElement, final XMLTag childTag)
	{
		...
		if(parentElement.getNodeName().equals("p") &&
			(childTag.getNodeName().equals("p") || childTag.getNodeName().equals("body")))  //TODO testing; comment
			return false; //TODO testing
		return true;  //TODO fix
	}
	...
	//TODO decide if we want to do this or not			document.appendChild(node);	//store the XML declaration in the document
			...
			if(docType!=null && !root.getNodeName().equals(docType.getName()))	//if we have a document type, and the root's name doesn't match what it calls for TODO only do this if we should validate, but perhaps *always* tidy if needed, whether we're validating or not
			{
				if(isTidy())  //if we're tidying
					...
					throw new XMLValidityException(XMLValidityException.ROOT_ELEMENT_TYPE, new Object[]{root.getNodeName(), docType.getName()}, reader.getLineIndex(), reader.getCharIndex(), reader.getName());	//show that this element type has already been declared TODO give a better indication of the element type declaration's location
			}
...
//TODO somewhere here, catch a sun.io.MalformedInputException when the input data is bad
		...
		{
			throw new XMLWellFormednessException(parseEOFException, ResourceBundle.getBundle(XMLResources.RESOURCE_BUNDLE_BASE_NAME).getString(XMLResources.RESOURCE_PREFIX+DOCUMENT_RESOURCE_ID), document.getNodeName()!=null ? document.getNodeName() : ResourceBundle.getBundle(XMLResources.RESOURCE_BUNDLE_BASE_NAME).getString(XMLResources.RESOURCE_PREFIX+UNKNOWN_RESOURCE_ID));	//show that we couldn't find the end of the document
		}
...
reader.tidy=isTidy();  //TODO fix
		...
		try
		{
			...
			{
				throw new XMLWellFormednessException(XMLWellFormednessException.UNEXPECTED_DATA, new Object[]{"", "</"+element.getNodeName()+">"}, reader.getLineIndex(), reader.getCharIndex(), reader.getName());	//show that we shouldn't have found the end of the element TODO use a constant here
			}
		}
		catch(final ParseUnexpectedDataException parseUnexpectedDataException)	//if we run into data we don't recognize
		{
...
*/
	...
	/**Resolves the namespace references in a document by iterating over each of
		its nodes and setting the namespace URIs and prefixes of the appropriate
	...
	*/
	...
	<ul>
		<li>NAMESPACE_ERR: Raised if the a <code>prefix</code> is
			...
			<code>null</code>, if a prefix is "xml" and the
			<code>namespaceURI</code> of the node is different from
			"http://www.w3.org/XML/1998/namespace", if a node is an attribute
			...
			or if a  node is an attribute and the <code>qualifiedName</code> of the
			node is "xmlns".</li>
	</ul>
	*/
...
//G***maybe combine all this into something that automatically updates attributes
		...
		while(attributeIterator.hasNext())  //while there are more attributes
		{
		  final XMLAttribute xmlAttribute=(XMLAttribute)attributeIterator.next();  //get the next attribute
			...
			final String attributeNamespaceURI=attributePrefix!=null ?
					getNamespaceURI(xmlElement, attributePrefix, true) : null;
