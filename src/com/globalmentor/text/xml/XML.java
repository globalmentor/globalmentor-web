...
public class XML
{
	...
	public final static String PUBLIC_ID_CHARS=SPACE_CHAR+CR_CHAR+LF_CHAR+"abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789-'()+,./:=?;!*#@$_%";
...
*/
	...
	/**The first set of characters which end a comment section
	(because the string "--" cannot appear inside a comment for campatibility reasons).*/
		...
		for(int i=0; i<childCount; ++i) //look at each of the children
		{
			...
			switch(childNode.getNodeType()) //see which type of node this is
			{
				case Node.TEXT_NODE:  //if this is a text node
					...
					break;
				case Node.ELEMENT_NODE: //if this is an element TODO what about CDATA?
...
//TODO del tabPos-=tabDelta;	//TODO check this; maybe static classes don't have recursive-aware functions
	...
	*/
	...
	@param mediaType The media type for which a default namespace should be found,
		or <code>null</code> if the media type is not known.
	@return The default XML namespace URI used by resources of the given content
		type, or <code>null</code> if there is no default namespace URI or the
		default namespace URI is not known.
	*/
	...
	/**Searches the attributes of the given node for a definition of a namespace
		...
		is defined, it is returned logically: a blank declared namespace will return
		<code>null</code>.
	...
	*/
	...
	/**Checks to ensure that all namespaces for the element and its attributes are properly declared using the appropriate <code>xmlns=</code> or <code>xmlns:prefix=</code> attribute declaration.
	The children of this element are optionally checked.
	@param element The element the namespace of which to declare.
	@param declarationElement The element on which to declare missing namespaces, or <code>null</code> if namespaces should always be declared on the element on which they are found missing.
	...
	*/
	...
	/**Checks to ensure that all namespaces for the <em>child elements</em> and
		their attributes are properly declared using the appropriate
		<code>xmlns=</code> or <code>xmlns:prefix=</code> attribute declaration.
		If a child element does not have a necessary namespace declaration,
		the declaration is added to the same parent element all the way down the
		hierarchy if there are no conflicts. If there is a conflict, the namespace
		is added to the child element itself.
		<p>This method is useful for adding namespace attributes to the top level
		of a fragment that contains unknown content that preferably should be
		lef undisturbed.</p>
	@param parentElement The element to which all child namespaces will be
		added if there are no conflicts.
	*/
	...
	/**Checks to ensure that all namespaces for the <em>child elements</em> and
		...
		of a fragment that contains unknown content that preferably should be
		lef undisturbed.</p>
	@param rootElement The element to which all child namespaces will be
		added if there are no conflicts.
	...
	*/
	...
	/**Declares a prefix for the given namespace using the appropriate
		<code>xmlns=</code> or <code>xmlns:prefix=</code> attribute declaration.
		<p>It is assumed that the namespace is used at the level of the given
		element. If the namespace prefix is already declared somewhere up the tree,
		and that prefix is assigned to the same namespace, no action occurs.</p>
	@param element The element for which the namespace should be declared.
	@param prefix The prefix to declare, or <code>null</code> if no prefix is
		used.
	@param namespaceURI The namespace being defined, or <code>null</code> if no
		namespace is used.
	*/
	...
	/**Determines if the given namespace is declared using the appropriate
		<code>xmlns=</code> or <code>xmlns:prefix=</code> attribute declaration
		either on the given element or on any element up the tree.
	<p>The "xmlns" and "xml" prefixes and namespaces always result in
		<code>true</code> being returned, because they never need to be declared.</p>
		@param element The element for which the namespace should be declared.
	@param prefix The prefix to declare, or <code>null</code> if no prefix is
		used.
	@param namespaceURI The namespace being defined, or <code>null</code> if no
		namespace is used.
	@return <code>true</code> if the namespace is sufficiently declared, either
		on the given element or somewhere up the element hierarchy.
	*/
	...
	/**Declares prefixes for the given namespaces using the appropriate
		<code>xmlns=</code> or <code>xmlns:prefix=</code> attribute declaration for
		the given element.
	@param declarationElement The element on which the namespaces should be declared.
	@param previxNamespacePairs An array of name/value pairs. The name of each is the 
		the prefix to declare, or <code>null</code> if no prefix is
		used. The value of each is the URI string of the namespace being defined,
		or <code>null</code> if no namespace is used.
	*/
