...
/**
 * A simple class that holds an external ID. This class does <em>not</em> represent a node in the XML document tree; it is only used internally for parsing. It
 * can only accessed by classes in its own package.
...
class XMLExternalID {
	...
	/**
	 * Default constructor which creates an external ID with <code>null</code> public and system IDs.
	 */
