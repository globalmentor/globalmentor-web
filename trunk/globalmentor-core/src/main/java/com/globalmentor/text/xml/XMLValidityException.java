...
public class XMLValidityException extends XMLParseException
{
	...
	*/
	public XMLValidityException(final short errorType, final Object[] messageArguments, final long lineIndex, final long charIndex, final String sourceName)
	{
		...
		super(ResourceBundle.getBundle(RESOURCE_BUNDLE_BASE_NAME).getString(RESOURCE_PREFIX+ERROR_NAME_RESOURCE_ID)+": "+MessageFormat.format(ResourceBundle.getBundle(RESOURCE_BUNDLE_BASE_NAME).getString(RESOURCE_PREFIX+errorType), messageArguments), lineIndex, charIndex, sourceName); //TODO use constant here
