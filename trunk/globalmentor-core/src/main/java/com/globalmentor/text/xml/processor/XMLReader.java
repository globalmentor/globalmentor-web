 ...
 * to convert all line-end markers to LF as XML specifies.
...
public class XMLReader extends ParseReader {
	...
	/**
	 * Processes newly buffered data to convert all end-of-line indicators to LF as specified by XML. If the last character in the buffer's new data (before the
	 * fetch index) is CR, we don't know if we should throw it away or replace it with an LF, so we'll move the fetch buffer back one so that it can be processed
	 * next time.
