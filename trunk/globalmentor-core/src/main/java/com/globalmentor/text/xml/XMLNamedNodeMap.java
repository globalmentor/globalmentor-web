...
{
	/**The delimiter used for forming combined namespaceURI+localName keys for
		the namespace map.
	*/
...
*/
	...
	@return <code>true</code> if a node exists in the map keyed to the given
	...
	*/
	...
	/**An extension to the DOM that indicates whether a certain node is currently
		in this named node map.
	@param arg The node that is checked to see if its name exists in the named node map.
	@return <code>true</code> if a node name of the given node exists in the named
	  node map, else <code>false</code>. Does not check to see if the value in the
	...
	*/
	...
	/**Adds a node using its <code>nodeName</code> attribute, which is the
	qualified name when applicable.<br/>
	...
	<ul>
		<li>WRONG_DOCUMENT_ERR: Raised if <code>arg</code> was created from a
			...
			<code>NamedNodeMap</code>.</li>
		<li>NO_MODIFICATION_ALLOWED_ERR: Raised if this
			<code>NamedNodeMap</code> is readonly.</li>
		<li>INUSE_ATTRIBUTE_ERR: Raised if <code>arg</code> is an
			<code>Attr</code> that is already an attribute of another
			<code>Element</code> object. The DOM user must explicitly clone
			<code>Attr</code> nodes to re-use them in other elements.</li>
	</ul>
	@version DOM Level 1
	*/
	...
	@param name The name of a node to remove. When this <code>NamedNodeMap</code>
		contains the attributes attached to an element, as returned by the attributes
		attribute of the <code>Node</code> interface, if the removed attribute is
	...
	*/
	...
	is less than zero or greater than or equal to the number of nodes in the map, this returns
	<code>null</code>.
	@param index Index into the map.
	@return The node at the <code>index</code>th position in the <code>NamedNodeMap</code>,
	...
	*/
	...
	@version DOM Level 1
	*/
	public int getLength() {return size();}	//return our size
