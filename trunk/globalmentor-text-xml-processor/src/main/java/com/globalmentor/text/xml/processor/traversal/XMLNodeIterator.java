...
package com.globalmentor.text.xml.processor.traversal;
...
public class XMLNodeIterator implements NodeIterator {
	...
	/**
	 * @return <code>true</code> if the iterator is invalid and is no longer attached to a root.
	 * @see #getRoot
	 */
	 ...
	 */
	protected void setReferenceNode(final Node newReferenceNode) {
	 ...
	 */
	public boolean isBeforeReferenceNode() {
		return beforeReferenceNode;
	}
	 ...
	 */
	public boolean isAfterReferenceNode() {
		return !beforeReferenceNode;
	}

	/**
	 * Sets whether the iterator is before the reference node.
	 * @param newBeforeReferenceNode Whether the iterator is now before the reference node.
	 * @see #setBeforeReferenceNode
	 */
	public void setBeforeReferenceNode(final boolean newBeforeReferenceNode) {
		beforeReferenceNode = newBeforeReferenceNode;
	}

	/**
	 * Sets whether the iterator is after the reference node.
	 * @param newAfterReferenceNode Whether the iterator is now after the reference node.
	 * @see #setAfterReferenceNode
	 */
	public void setAfterReferenceNode(final boolean newAfterReferenceNode) {
		beforeReferenceNode = !newAfterReferenceNode;
	}
	...
	/**
	 * Creates a node iterator, specifying the root and the nodes to show and not expanding entity references.
	 * @param rootNode The root node over which to iterate.
			...
			if(oldReferenceNode.hasChildNodes()) { //if the reference node has children
				...
				while(newReferenceNode == null) { //while we haven't found a new reference node
					...
					if(currentReferenceNode != null && currentReferenceNode != getRoot()) {
						...
						//if we couldn't find a parent, or we found the root, we're at the end of the iterator
						break;
				}
				...
				//if we tried to go past a childless root
